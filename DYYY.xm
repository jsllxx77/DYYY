...
// 根因：分享面板宿主 DUXSheetNavigationController + DUXVisualEffectView（原生 UIKit modal），
// 官方关闭转场极快（dismiss 后约 176ms 即移除，服务端 dpp_panel_dismiss_velocity_duration=0.16）。
// 方案：接管 dismiss，自定义 0.35s 慢速下滑关闭 + 遮罩淡出。
// 注意：hook 注册必须用 class_addMethod（只添加目标类自身方法），
// 禁止 method_setImplementation 直接改继承链方法（会污染 UIView 基类）。
...
// 维护活动面板引用：DUXVisualEffectView 进入/离开 window 时更新（dismiss 接管依赖）
...
// 注册面板 window 跟踪（class_addMethod 仅添加 DUXVisualEffectView 自身方法，不影响 UIView 基类）
static void DYYYInstallDUXTraceHooks(void) {
    ...
    if (duxClass) {
        ...
        if (m) {
            ...
            class_addMethod(duxClass, @selector(didMoveToWindow), (IMP)DYYYTraceDUXDidMoveToWindow, "v@:");
        }
    }
}
